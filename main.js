...
1. double the num immediately before checkNum (right most num), skipping every other num to the left until reaching beginning of arr
...
5. if (totalSum % 10 == 0) return true (valid CC), else false (invalid CC)
*/
...
const findInvalidCards = arrayOfCards => {
    let invalidCards = []
    //iterate through each card in the batch, plug into validateCred function
    //if its an invalid card, add it to the invalidCards array
...
//create a variable to be passed to the idIndividualCardCompanies function
